...
function exitOnMismatch(key, fileConfig) {
  const expectedKeyType = getTypeOf(config[key]);
  const actualKeyType = getTypeOf(fileConfig[key]);
...
const validators = {
  ...
  extensions: (fileConfig) => {
    const extensions = Object.keys(fileConfig.extensions);

    extensions.forEach((extension) => {
      const code = fileConfig.extensions[extension];
